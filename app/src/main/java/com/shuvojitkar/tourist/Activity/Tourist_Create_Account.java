...
public class Tourist_Create_Account extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_tourist_create_account);
        cn = this;
        init();
        ...
        mFirebaseAuth.createUserWithEmailAndPassword(email,password).addOnCompleteListener(new OnCompleteListener<AuthResult>() {
            ...
            public void onComplete( Task<AuthResult> task) {
                if(task.isSuccessful()){
                    ...
                    alertDialog.setMessage("You Select :"+"tourist");
                    alertDialog.setButton(AlertDialog.BUTTON_POSITIVE, "Ok",
                            new DialogInterface.OnClickListener() {
                                public void onClick(DialogInterface dialog, int which) {
                                    FirebaseUser currentuser = FirebaseAuth.getInstance().getCurrentUser();
                                    String Uid = currentuser.getUid();
                                    mDatabaseReference = GetFirebaseRef.GetDbIns().getReference().child("tourist").child(Uid);
                                    HashMap<String,String> map= new HashMap<String, String>();
                                    map.put("name",display_name);
                                    map.put("email",email);
                                    map.put("password",password);
                                    map.put("type","tourist");
                                    ...
                                    mDatabaseReference.setValue(map).addOnCompleteListener(new OnCompleteListener<Void>() {
                                        ...
                                        public void onComplete(@NonNull Task<Void> task) {
                                            if (task.isSuccessful()){
                                                ...
                                                startActivity(new Intent(Tourist_Create_Account.this,MainActivity.class));
    ...
    private void init() {
        mDisplayName = (TextInputLayout) findViewById(R.id.tourist_reg_display_name);
        mEmail = (TextInputLayout) findViewById(R.id.tourist_reg_email);
        mPassowrd = (TextInputLayout) findViewById(R.id.tourist_reg_password);
        mCreateAccountBtn = (Button) findViewById(R.id.tourist_reg_create_btn);
